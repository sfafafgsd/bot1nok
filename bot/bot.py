...
def get_repl_logs(update: Update, context):
    result = "Логи репликацией:\n"
    with open("/plogs/postgresql.log") as log_reader:
        s = log_reader.readline()
        while s != "":
            if s.find("repl") != -1:
                result += s + '\n'
            s = log_reader.readline()
    #result = exec_remote_cmd("cat /var/log/postgresql/postgresql-15-main.log | grep \"repl_user\"")
    send_msg_by_chunks(result, update)
